...
const DEFAULT_PROPS: Record<SectionType, Record<string, string | number | boolean>> = {
  ...
  hero: {
    ...
    body_en: "Browse 10,000+ properties to find the perfect one for you. Buy, rent or invest with confidence.",
    buttons: "Одоо хайх|Search Now|/search|primary\nДэлгэрэнгүй|Learn More|/about|outline",
    bgImage: "",
    bgOverlay: "#14532d99",
    bgColor: "#166534",
    textColor: "#ffffff",
...
type FieldType = "text" | "textarea" | "color" | "url" | "select" | "range" | "buttons" | "navlinks";
interface FieldDef { key: string; label: string; type: FieldType; options?: string[]; min?: number; max?: number }
...
const FIELD_DEFS: Record<SectionType, FieldDef[]> = {
  navbar: [
    ...
    { key: "showLangSwitch", label: "Хэлний солигч", type: "select", options: ["true", "false"] },
    { key: "bgColor",        label: "Арын өнгө",     type: "color" },
    { key: "textColor",      label: "Текстийн өнгө", type: "color" },
  ],
  slider: [
    { key: "slides",       label: "Слайдууд [МН] (мөр бүр: URL|гарчиг|дэд гарчиг|товч)", type: "textarea" },
    { key: "slides_en",    label: "Слайдууд [EN] (мөр бүр: URL|title|subtitle|btn)",       type: "textarea" },
    { key: "height",       label: "Өндөр (px)",                  type: "range", min: 300, max: 800 },
    { key: "overlayColor", label: "Зургийн бүрхүүлийн өнгө",    type: "color" },
    { key: "textColor",    label: "Текстийн өнгө",               type: "color" },
    { key: "autoPlay",     label: "Автомат тоглуулах",           type: "select", options: ["true", "false"] },
    { key: "interval",     label: "Шилжих хугацаа (ms)",         type: "range", min: 1000, max: 8000 },
  ],
  hero: [
  ...
  features: [
    ...
    { key: "textColor",  label: "Текстийн өнгө",               type: "color" },
    { key: "items",      label: "Зүйлүүд [МН] (мөр бүр: emoji гарчиг|тайлбар)", type: "textarea" },
    { key: "items_en",   label: "Зүйлүүд [EN] (мөр бүр: emoji title|description)", type: "textarea" },
  ],
  cta: [
    { key: "heading",        label: "Гарчиг [МН]",              type: "text" },
    { key: "heading_en",     label: "Гарчиг [EN]",              type: "text" },
...
function serializeButtons(btns: ButtonDef[]): string {
  return btns.map(b => `${b.text}|${b.text_en}|${b.url}|${b.style}`).join("\n");
}
...
function defaultSectionsForPage(slug: string, customTitle?: string): Section[] {
  const nav    = { id: `nav-${slug}`,    type: "navbar"   as SectionType, props: { ...DEFAULT_PROPS.navbar } };
  const footer = { id: `footer-${slug}`, type: "footer"   as SectionType, props: { ...DEFAULT_PROPS.footer } };
  const pageTitle = customTitle ?? DEFAULT_PAGE_LIST.find(p => p.slug === slug)?.title ?? slug;

  if (slug === "home") {
    return [
      nav,
      { id: `slider-${slug}`, type: "slider"   as SectionType, props: { ...DEFAULT_PROPS.slider } },
      { id: `feat-${slug}`,   type: "features" as SectionType, props: { ...DEFAULT_PROPS.features } },
      { id: `cta-${slug}`,    type: "cta"      as SectionType, props: { ...DEFAULT_PROPS.cta } },
      footer,
    ];
  }
  return [
    nav,
    { id: `hero-${slug}`, type: "hero" as SectionType, props: { ...DEFAULT_PROPS.hero, heading: pageTitle } },
    footer,
  ];
}
...
export default function BuilderPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [templateName, setTemplateName] = useState(() => {
    if (typeof window !== "undefined") {
      try { return JSON.parse(localStorage.getItem(STORAGE_KEY(id)) || "{}").templateName || ""; } catch {}
    }
  ...
  const [theme, setTheme] = useState<GlobalTheme>(() => {
    if (typeof window !== "undefined") {
      try { return JSON.parse(localStorage.getItem(STORAGE_KEY(id)) || "{}").theme || DEFAULT_THEME; } catch {}
    }
  ...
  const [pageList, setPageList] = useState<PageDef[]>(() => {
    if (typeof window !== "undefined") {
      try {
        const stored = JSON.parse(localStorage.getItem(STORAGE_KEY(id)) || "{}");
  ...
  const [allPages, setAllPages] = useState<Record<string, Section[]>>(() => {
    ...
    return Object.fromEntries(DEFAULT_PAGE_LIST.map(p => [p.slug, defaultSectionsForPage(p.slug)]));
  });
  ...
  useEffect(() => {
    localStorage.setItem(STORAGE_KEY(id), JSON.stringify({ templateName, theme, allPages, pageList }));
    setSaved(true);
  ...
  function addPage(title: string, title_en: string) {
    ...
    const newPage: PageDef = { slug, title: title || "Шинэ хуудас", title_en: title_en || title || "New Page" };
    setPageList(pl => [...pl, newPage]);
    setAllPages(ap => ({ ...ap, [slug]: defaultSectionsForPage(slug, newPage.title) }));
    setCurrentPage(slug);
    setSelectedId(null);
  }
  ...
  function updateProps(sid: string, patch: Record<string, string | number | boolean>) {
    setSections((ss) => ss.map((s) => s.id === sid ? { ...s, props: { ...s.props, ...patch } } : s));
  }
  ...
  function addSection(type: SectionType) {
    ...
    setSections((ss) => [...ss, { id: newId, type, props: { ...DEFAULT_PROPS[type] } }]);
    setSelectedId(newId);
  }

  function exportJson() {
    const blob = new Blob([JSON.stringify({ templateName, theme, allPages }, null, 2)], { type: "application/json" });
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-950 text-white overflow-hidden">
      ...
      <header className="h-12 bg-slate-900 border-b border-slate-800 flex items-center gap-3 px-4 flex-shrink-0">
        ...
        {saved && <span className="text-xs text-green-500 flex items-center gap-1"><span className="w-1.5 h-1.5 bg-green-500 rounded-full inline-block" />Хадгалагдлаа</span>}
        <a
        ...
        <button onClick={exportJson} className="flex items-center gap-1.5 text-xs bg-slate-700 hover:bg-slate-600 px-3 py-1.5 rounded-lg transition-colors">
          <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="flex flex-1 overflow-hidden">
        <LeftPanel
          ...
          pageList={pageList}
          onPageChange={(slug) => { setCurrentPage(slug); setSelectedId(null); }}
          onAddPage={addPage}
        ...
        <div className="flex-1 overflow-y-auto bg-slate-800 p-4 sm:p-6">
          <div className="mx-auto rounded-xl overflow-hidden shadow-2xl ring-1 ring-white/10" style={{ maxWidth: 960, fontFamily: theme.fontFamily }}>
            {sections.length === 0 ? (
...
function CanvasSection({ section, theme, isSelected, isFirst, isLast, onSelect, onDelete, onMove, onAddAfter }: {
  section: Section; theme: GlobalTheme;
  isSelected: boolean; isFirst: boolean; isLast: boolean;
  ...
  return (
    ...
    >
      {/* Section label badge */}
      <div className={`absolute top-2 left-2 z-20 flex items-center gap-1 transition-opacity ${isSelected ? "opacity-100" : "opacity-0 group-hover:opacity-100"}`}>
      ...
      >
        ...
        <div className="relative">
          <button
            onClick={() => { setConfirmDel(false); setAddOpen(v => !v); }}
            ...
            <>
              ...
              <div className="absolute top-full mt-1 right-0 z-40 bg-slate-800 border border-slate-700 rounded-xl overflow-hidden shadow-2xl w-40">
                <div className="px-3 py-2 border-b border-slate-700">
                  <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Нэмэх</p>
                </div>
                {(Object.keys(SECTION_LABELS) as SectionType[]).map((t) => (
                  <button key={t} onClick={() => { onAddAfter(t); setAddOpen(false); }}
        ...
        {confirmDel ? (
          ...
          <button
            onClick={() => { setAddOpen(false); setConfirmDel(true); }}
...
function LeftPanel({ theme, sections, selectedId, currentPage, pageList, onPageChange, onAddPage, onDeletePage, onThemeChange, onSelect, onMove, onDelete, onAdd }: {
  theme: GlobalTheme; sections: Section[]; selectedId: string | null;
  currentPage: string; pageList: PageDef[];
  onPageChange: (slug: string) => void;
  onAddPage: (title: string, title_en: string) => void;
  onDeletePage: (slug: string) => void;
  onThemeChange: (p: Partial<GlobalTheme>) => void; onSelect: (id: string) => void;
  onMove: (id: string, dir: -1 | 1) => void; onDelete: (id: string) => void; onAdd: (type: SectionType) => void;
}) {
  const [addOpen, setAddOpen] = useState(false);
      ...
      <div className="flex-1 overflow-y-auto">
        ...
        <div className="border-b border-slate-800">
          <div className="flex items-center justify-between px-4 py-3">
            <button onClick={() => setPagesOpen(v => !v)} className="flex items-center gap-1.5 text-xs font-semibold text-slate-400 uppercase tracking-wider hover:text-white transition-colors flex-1 text-left">
              Хуудсууд
              <svg className={`w-3 h-3 transition-transform ${pagesOpen ? "rotate-180" : ""}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
              </svg>
            </button>
            <button
              onClick={() => { setPagesOpen(true); setAddingPage(v => !v); }}
              ...
              {pageList.map(page => (
                <div
                  ...
                  className={`flex items-center gap-1 rounded-lg group transition-colors ${
                    currentPage === page.slug ? "bg-green-600" : "hover:bg-slate-800"
                  }`}
                >
                  <button
                    ...
                    className={`flex-1 flex items-center gap-2 px-2 py-2 text-xs font-medium text-left transition-colors ${
                      currentPage === page.slug ? "text-white" : "text-slate-400 hover:text-white"
                    }`}
                  >
              ...
              {addingPage && (
                <div className="mt-2 space-y-1.5 px-1">
                  <input
                    ...
                    onKeyDown={(e) => { if (e.key === "Enter") confirmAddPage(); if (e.key === "Escape") setAddingPage(false); }}
                    placeholder="Нэр (МН)"
                    className={inp}
                  />
                  <input
                  ...
                  <div className="flex gap-1">
                    <button onClick={confirmAddPage} className="flex-1 py-1.5 text-xs bg-green-600 hover:bg-green-500 text-white rounded-lg transition-colors font-medium">
                      Нэмэх
                    </button>
                    <button onClick={() => setAddingPage(false)} className="flex-1 py-1.5 text-xs bg-slate-700 hover:bg-slate-600 text-slate-300 rounded-lg transition-colors">
                      Болих
                    </button>
                  </div>
                </div>
        ...
        {/* Theme */}
        <div className="border-b border-slate-800">
          <button onClick={() => setThemeOpen((v) => !v)} className="w-full flex items-center justify-between px-4 py-3 text-xs font-semibold text-slate-400 uppercase tracking-wider hover:text-white transition-colors">
            Ерөнхий загвар
        ...
        <div>
          ...
          <div className="px-2 pb-4 relative">
            <button onClick={() => setAddOpen((v) => !v)} className="w-full flex items-center gap-1.5 px-3 py-2 rounded-lg border border-dashed border-slate-700 text-slate-500 hover:border-slate-500 hover:text-slate-300 text-xs transition-colors">
              <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {addOpen && (
              <>
                ...
                <div className="absolute bottom-full mb-1 left-0 right-0 z-20 bg-slate-800 border border-slate-700 rounded-lg overflow-hidden shadow-xl">
                  {(Object.keys(SECTION_LABELS) as SectionType[]).map((t) => (
                    <button key={t} onClick={() => { onAdd(t); setAddOpen(false); }} className="w-full text-left px-3 py-2 text-xs text-slate-300 hover:bg-slate-700 hover:text-white transition-colors">
                      {SECTION_LABELS[t]}
                    </button>
                  ))}
                </div>
...
function NavLinkEditor({ value, pageList, onChange }: {
  value: string;
  pageList: PageDef[];
  onChange: (v: string) => void;
}) {
  const links = parseNavLinks(value);
  const inp = "w-full bg-slate-700 border border-slate-600 rounded-md px-2 py-1 text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-green-500 transition";

  function update(i: number, patch: Partial<NavLinkDef>) {
    onChange(serializeNavLinks(links.map((l, idx) => idx === i ? { ...l, ...patch } : l)));
  }

  function remove(i: number) {
    onChange(serializeNavLinks(links.filter((_, idx) => idx !== i)));
  }

  function add() {
    onChange(serializeNavLinks([...links, { text: "Холбоос", text_en: "Link", url: "#" }]));
  }

  return (
    <div className="space-y-2">
      {links.map((l, i) => (
        <div key={i} className="border border-slate-700 rounded-lg p-2.5 space-y-1.5 bg-slate-800/50">
          <div className="flex items-center justify-between mb-0.5">
            <span className="text-xs text-slate-400 font-medium">Холбоос {i + 1}</span>
            <button onClick={() => remove(i)} className="text-slate-600 hover:text-red-400 transition-colors text-sm leading-none">×</button>
          </div>
          <input value={l.text} onChange={e => update(i, { text: e.target.value })} placeholder="Нэр [МН]" className={inp} />
          <input value={l.text_en} onChange={e => update(i, { text_en: e.target.value })} placeholder="Name [EN]" className={inp} />
          {/* Route selector */}
          <div className="space-y-1">
            <select
              value={pageList.find(p => "/" + p.slug === l.url || p.slug === l.url.replace(/^\//, ""))?.slug ? l.url : "__custom__"}
              onChange={e => {
                if (e.target.value !== "__custom__") update(i, { url: e.target.value });
              }}
              className={inp}
            >
              <option value="__custom__">— Хуудас сонгох —</option>
              {pageList.map(p => (
                <option key={p.slug} value={"/" + p.slug}>/{p.slug} ({p.title})</option>
              ))}
            </select>
            <input
              value={l.url}
...
function ButtonEditor({ value, onChange }: { value: string; onChange: (v: string) => void }) {
  const buttons = parseButtons(value);
  const inp = "w-full bg-slate-700 border border-slate-600 rounded-md px-2 py-1 text-xs text-white placeholder-slate-500 focus:outline-none focus:ring-1 focus:ring-green-500 transition";

  function update(index: number, patch: Partial<ButtonDef>) {
    const updated = buttons.map((b, i) => i === index ? { ...b, ...patch } : b);
    onChange(serializeButtons(updated));
  }

  function remove(index: number) {
    onChange(serializeButtons(buttons.filter((_, i) => i !== index)));
  }

  function add() {
    onChange(serializeButtons([...buttons, { text: "Товч", text_en: "Button", url: "#", style: "primary" }]));
  }

  const STYLES: ButtonDef["style"][] = ["primary", "outline", "ghost"];
  const STYLE_LABELS = { primary: "Үндсэн", outline: "Хүрээтэй", ghost: "Тунгалаг" };
  const STYLE_COLORS = { primary: "#16a34a", outline: "transparent", ghost: "transparent" };
      ...
      {buttons.map((b, i) => (
        <div key={i} className="border border-slate-700 rounded-lg p-2.5 space-y-1.5 bg-slate-800/50">
          <div className="flex items-center justify-between mb-1">
            <span className="text-xs text-slate-400 font-medium">Товч {i + 1}</span>
            <button onClick={() => remove(i)} className="text-slate-600 hover:text-red-400 transition-colors text-sm leading-none">×</button>
          </div>
          <input value={b.text} onChange={e => update(i, { text: e.target.value })} placeholder="Текст [МН]" className={inp} />
          <input value={b.text_en} onChange={e => update(i, { text_en: e.target.value })} placeholder="Text [EN]" className={inp} />
          <input value={b.url} onChange={e => update(i, { url: e.target.value })} placeholder="URL эсвэл /route" className={inp} />
          ...
          <div className="pt-1 flex justify-center">
            <button style={{
              ...
              padding: "5px 14px", borderRadius: 7, fontSize: 11, fontWeight: 700, cursor: "default",
            }}>{b.text || "Товч"}</button>
          </div>
        </div>
      ))}
...
function RightPanel({ section, pageList, onChange, onDelete, onAdd }: {
        ...
        {addOpen && (
          <>
            ...
            <div className="absolute bottom-full mb-1 left-3 right-3 z-20 bg-slate-800 border border-slate-700 rounded-xl overflow-hidden shadow-2xl">
              <div className="px-3 py-2 border-b border-slate-700">
                <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Хэсэг сонгох</p>
              </div>
              {(Object.keys(SECTION_LABELS) as SectionType[]).map((t) => (
                <button key={t} onClick={() => { onAdd(t); setAddOpen(false); }}
    ...
    <div className="w-72 flex-shrink-0 bg-slate-900 border-l border-slate-800 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 pb-2">
        {FIELD_DEFS[section.type].map((field) => {
          ...
          if (field.type === "navlinks") return (
            <div key={field.key}>
              <label className="block text-xs text-slate-500 mb-2">{field.label}</label>
              <NavLinkEditor value={val} pageList={pageList} onChange={(v) => onChange({ [field.key]: v })} />
            </div>
          );
          ...
          if (field.type === "buttons") return (
            <div key={field.key}>
              <label className="block text-xs text-slate-500 mb-2">{field.label}</label>
              <ButtonEditor value={val} onChange={(v) => onChange({ [field.key]: v })} />
            </div>
          );
          ...
          if (field.type === "textarea") return (
            <div key={field.key}>
              <label className="block text-xs text-slate-500 mb-1">{field.label}</label>
              <textarea value={val} onChange={(e) => onChange({ [field.key]: e.target.value })} rows={field.key === "slides" || field.key === "items" ? 8 : 3} className={inp + " resize-y"} />
            </div>
          );
          ...
          if (field.type === "range") return (
            <div key={field.key}>
              <label className="block text-xs text-slate-500 mb-1">{field.label}: <span className="text-white">{val}</span></label>
              <input type="range" min={field.min ?? 0} max={field.max ?? 100} value={Number(val) || field.min} onChange={(e) => onChange({ [field.key]: e.target.value })} className="w-full accent-blue-500" />
            </div>
          );
...
function SectionPreview({ section, theme }: { section: Section; theme: GlobalTheme }) {
  ...
  switch (section.type) {
    case "navbar": {
      ...
      return (
        <nav style={{ background: String(p.bgColor), color: String(p.textColor), fontFamily: font, padding: "14px 32px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          <span style={{ fontSize: 18, fontWeight: 800, color: theme.primaryColor }}>{String(p.logo)}</span>
          <div style={{ display: "flex", gap: 20 }}>
            {navLinks.map((l, i) => <span key={i} style={{ fontSize: 13, opacity: 0.75 }}>{l.text}</span>)}
          </div>
    ...
    case "slider": {
      ...
      const [imgUrl, title = "Слайдер гарчиг", sub = ""] = firstSlide.split("|");
      const h = Number(p.height) || 400;
      return (
        <div style={{ height: Math.min(h, 300), position: "relative", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
          {imgUrl?.startsWith("http") && <img src={imgUrl} alt="" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover" }} />}
          <div style={{ position: "absolute", inset: 0, background: String(p.overlayColor || "#00000060") }} />
          <div style={{ position: "relative", textAlign: "center", color: String(p.textColor || "#fff"), padding: "0 32px" }}>
            <p style={{ fontSize: 11, opacity: 0.7, marginBottom: 4 }}>Слайд 1 / {String(p.slides).split("\n").filter(Boolean).length}</p>
            <div style={{ fontSize: Math.min(theme.headingSize * 0.7, 32), fontWeight: 800, marginBottom: 8 }}>{title}</div>
            {sub && <div style={{ fontSize: 14, opacity: 0.85 }}>{sub}</div>}
            <div style={{ display: "flex", gap: 6, justifyContent: "center", marginTop: 16 }}>
              {String(p.slides).split("\n").filter(Boolean).map((_, i) => (
                <div key={i} style={{ width: i === 0 ? 20 : 7, height: 7, borderRadius: 4, background: i === 0 ? "#fff" : "rgba(255,255,255,0.4)" }} />
              ))}
            </div>
          </div>
        </div>
    ...
    case "hero": {
      ...
      return (
        <section style={{ ...style, color: String(p.textColor), fontFamily: font, padding: "64px 40px", textAlign: align }}>
          <div style={{ maxWidth: 640, margin: align === "center" ? "0 auto" : align === "right" ? "0 0 0 auto" : "0" }}>
            <h1 className={animClass} style={{ fontSize: theme.headingSize * 0.8, fontWeight: 900, lineHeight: 1.15, margin: "0 0 12px" }}>{String(p.heading)}</h1>
            <p className={animClass} style={{ fontSize: theme.headingSize * 0.36, fontWeight: 500, margin: "0 0 10px", opacity: 0.88, animationDelay: "60ms" }}>{String(p.subheading)}</p>
            <p className={animClass} style={{ fontSize: theme.bodySize, margin: "0 0 24px", opacity: 0.72, animationDelay: "120ms" }}>{String(p.body)}</p>
            <div style={{ display: "flex", gap: 10, justifyContent: justifyMap[align], flexWrap: "wrap" }}>
              {parseButtons(String(p.buttons || "")).map((b, i) => (
                <button key={i} style={{
                  background: b.style === "primary" ? "#fff" : "transparent",
                  color: b.style === "primary" ? theme.primaryColor : String(p.textColor),
                  border: b.style === "ghost" ? "none" : `2px solid ${b.style === "primary" ? "#fff" : String(p.textColor)}60`,
                  fontSize: 13, fontWeight: 700, padding: "10px 22px", borderRadius: 8, cursor: "pointer",
                }}>{b.text}</button>
              ))}
            </div>
          </div>
    ...
    case "features": {
      const items = String(p.items || "").split("\n").filter(Boolean).map((line) => {
        ...
        return { icon: parts[0] ?? "", title: parts.slice(1).join(" "), desc: desc.trim() };
      });
      const style = p.bgImage ? {
      ...
      return (
        <section style={{ ...style, fontFamily: font, padding: "56px 40px" }}>
          <div style={{ textAlign: "center", marginBottom: 36 }}>
            <h2 className={animClass} style={{ fontSize: theme.headingSize * 0.6, fontWeight: 800, color: String(p.titleColor), margin: "0 0 8px" }}>{String(p.title)}</h2>
            <p style={{ fontSize: theme.bodySize - 1, color: String(p.textColor) }}>{String(p.subtitle)}</p>
          </div>
          <div style={{ display: "grid", gridTemplateColumns: `repeat(${Math.min(items.length, 3)}, 1fr)`, gap: 18 }}>
            {items.map((item, i) => (
              <div key={i} className={animClass} style={{ background: "rgba(255,255,255,0.9)", borderRadius: 14, padding: "22px 18px", boxShadow: "0 1px 4px rgba(0,0,0,0.07)", animationDelay: `${i * 60}ms` }}>
                <div style={{ fontSize: 28, marginBottom: 8 }}>{item.icon}</div>
                <h3 style={{ fontWeight: 700, fontSize: theme.bodySize, color: String(p.titleColor), margin: "0 0 4px" }}>{item.title}</h3>
                <p style={{ fontSize: theme.bodySize - 2, color: String(p.textColor), lineHeight: 1.5, margin: 0 }}>{item.desc}</p>
              </div>
            ))}
          </div>
        </section>
    ...
    case "cta": {
      ...
      return (
        <section style={{ ...style, color: String(p.textColor), fontFamily: font, padding: "64px 40px", textAlign: "center" }}>
          <h2 className={animClass} style={{ fontSize: theme.headingSize * 0.6, fontWeight: 800, margin: "0 0 12px" }}>{String(p.heading)}</h2>
          <p style={{ fontSize: theme.bodySize, margin: "0 0 28px", opacity: 0.82 }}>{String(p.subheading)}</p>
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            {parseButtons(String(p.buttons || "")).map((b, i) => (
              <button key={i} style={{
                background: b.style === "primary" ? theme.primaryColor : "transparent",
                color: b.style === "primary" ? "#fff" : String(p.textColor),
                border: b.style === "ghost" ? "none" : `2px solid ${b.style === "primary" ? theme.primaryColor : String(p.textColor)}60`,
                fontSize: 14, fontWeight: 700, padding: "12px 28px", borderRadius: 9, cursor: "pointer",
              }}>{b.text}</button>
            ))}
          </div>
        </section>
    ...
    case "footer":
      return (
        <footer style={{ background: String(p.bgColor), fontFamily: font, padding: "32px 40px" }}>
          <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", flexWrap: "wrap", gap: 14, marginBottom: 16 }}>
            <span style={{ fontSize: 15, fontWeight: 800, color: theme.primaryColor }}>{String(p.brand)}</span>
            <div style={{ display: "flex", gap: 18 }}>
              {String(p.links).split(",").map((l, i) => <span key={i} style={{ fontSize: 12, color: String(p.textColor), opacity: 0.8 }}>{l.trim()}</span>)}
            </div>
          </div>
          <p style={{ fontSize: 11, color: String(p.textColor), opacity: 0.5, margin: 0 }}>{String(p.copyright)}</p>
        </footer>
      );
